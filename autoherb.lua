--[[ 
MINI BOT + MAC-LIKE UI + ESP (Rarity 3–5)  •  สำหรับทดสอบใน private server เท่านั้น
ฟีเจอร์:
  • เปิดดาบ → TP ไปเป้า → เก็บ (Remote+Prompt) → กลับจุด HOME
  • UI เหมือนภาพ: ปุ่ม Hover Shade / Outline, สไลเดอร์, เช็กบ็อกซ์, แถบข้าง/ค้นหา
  • ESP: Billboard + Highlight โชว์ชื่อ/ระยะ/Δh, อัปเดตตามจริง
  • Motion แพตช์: snap พื้น, LoS adaptive, จำกัดการตกต่อฮอพ, watchdog กู้ค้าง/แกว่ง
  • ปรับพารามิเตอร์จาก UI (สไลเดอร์ควบคุม TP_STEP_STUDS, Outline เปิด/ปิด ESP highlight, check = Auto)

ติดตั้ง: วางเป็น LocalScript ใน StarterPlayerScripts หรือ StarterGui
]]--
...
--== Config ==--
local ROOT_NAME             = "Resources"                          -- โฟลเดอร์ทรัพยากร
local HOME_POS              = Vector3.new(-519.435,-5.452,-386.665) -- จุดกลับ
local SPEED_STUDS_PER_S     = 150
local MIN_TWEEN_TIME        = 0.08
local TP_STEP_STUDS         = 90   -- ปรับได้จากสไลเดอร์ (60..140)
local SAFE_Y_OFFSET         = 1.5  -- ใช้เฉพาะระหว่างทางและเฉพาะ LoS ไม่ผ่าน
...
local function mk(c,p,props) local i=Instance.new(c); i.Parent=p; for k,v in pairs(props or {}) do i[k]=v end; return i end

--== Motion helpers ==--
local function getHRP()
	if HRP and HRP.Parent then return HRP end
	if LP.Character then HRP = LP.Character:FindFirstChild("HumanoidRootPart") or LP.Character:WaitForChild("HumanoidRootPart",5) end
	return HRP
end
local function zeroVel(h) if not h then return end h.AssemblyLinearVelocity=Vector3.zero; h.AssemblyAngularVelocity=Vector3.zero end
...
local function hasLineOfSight(fromPos,toPos) return workspace:Raycast(fromPos,toPos-fromPos,RP_BLACKLIST)==nil end
local function snapToFloor(pos, up, down)
	...
	if res then lastGroundY = res.Position.Y+0.10; return Vector3.new(pos.X,lastGroundY,pos.Z) end
	if lastGroundY then return Vector3.new(pos.X,lastGroundY,pos.Z) end
	return pos
end
local function losAround(fromPos, center, r)
	r = r or 3
	local offs = {Vector3.new(0,0,0),Vector3.new(r,0,0),Vector3.new(-r,0,0),Vector3.new(0,0,r),Vector3.new(0,0,-r),Vector3.new(0,r,0)}
	for _,o in ipairs(offs) do if hasLineOfSight(fromPos, center+o) then return true end end
	return false
end
...
local function tweenHop(toPos)
	...
	while currentTween and currentTween.PlaybackState==Enum.PlaybackState.Playing do
		local dt=RunService.Heartbeat:Wait(); elapsed+=dt
		if elapsed>t+2 then currentTween:Cancel(); h.CFrame=CFrame.new(snapToFloor(toPos)); break end
	end
	zeroVel(h)
end
local function tweenTP(targetPos)
	...
	if not hasLineOfSight(startPos, targetPos) then targetPos = targetPos + Vector3.new(0,HEIGHT_BOOST,0) end
	...
	for i=1,steps do
		...
		if drop>MAX_DROP_PER_HOP then
			local mid=Vector3.new(p.X, cur.Y-MAX_DROP_PER_HOP, p.Z); tweenHop(mid); zeroVel(getHRP())
		end
...
local function tpTo(vec3)
	...
	ensureMobile()
	local h=getHRP()
	if h and lastTP and ((lastTP - vec3).Magnitude < 1.0) and (math.abs(lastTP.Y - vec3.Y) < 0.2) then _tpLock=false; return end
...
local function _normalizeId(id) if not id or type(id)~="string" then return nil end id=id:gsub("%s+",""); if id=="" then return nil end if id:sub(1,1)~="{" then id="{ "..id.." }":gsub(" ","") end return id end
local function _findCollectIdFromInst(inst)
	...
	for _,d in ipairs(inst:GetDescendants()) do
		if d:IsA("StringValue") then local n=d.Name:lower()
...
local function nearestPrompt(inst)
	...
	for _,d in ipairs(inst:GetDescendants()) do
		if d:IsA("ProximityPrompt") and d.Enabled then
			local pos
			if d.Parent and d.Parent:IsA("BasePart") then pos=d.Parent.Position else
				local ok,cf=pcall(function() return inst:GetModelCFrame() end); pos=ok and cf.Position or h.Position
			end
...
local function pressPrompt(prompt)
	...
	else local t0=os.clock(); pcall(function() fireproximityprompt(prompt,1) end); while os.clock()-t0<hd+0.05 do task.wait() end end
	return true
end
...
local function attach(inst)
	...
	local function bindWhenPartReady()
		...
		if not part then
			inst.ChildAdded:Connect(function() if not targets[part] then bindWhenPartReady() end end)
			return
		end
...
workspace.ChildAdded:Connect(function(c)
	if c.Name==ROOT_NAME then
		ROOT=c; for part,rec in pairs(targets) do pcall(function() if rec.bb then rec.bb:Destroy() end end); pcall(function() if rec.hl then rec.hl:Destroy() end end) end
		targets = {}
		for _,d in ipairs(ROOT:GetDescendants()) do if d:GetAttribute("Rarity") ~= nil then attach(d) end end
		ROOT.DescendantAdded:Connect(function(d) if d:GetAttribute("Rarity") ~= nil then attach(d) end end)
	end
...
-- เลือกเป้า (ใกล้สุด + ไม่ถูก cooldown)
...
-- HOME ติดพื้นเริ่มต้น + auto refresh Y
...
mk("UIPadding", sidebar, {PaddingTop=UDim.new(0,12), PaddingLeft=UDim.new(0,14), PaddingRight=UDim.new(0,10)}); mk("UIListLayout", sidebar, {Padding=UDim.new(0,8)})
local function sideItem(text,icon) local row=mk("Frame", sidebar, {Size=UDim2.new(1,0,0,28), BackgroundTransparency=1})
	mk("TextLabel", row, {Size=UDim2.new(1,0,1,0), BackgroundTransparency=1, Text=((icon and (icon.."  ")) or "")..text, Font=Enum.Font.Gotham, TextSize=14, TextColor3=color.muted, TextXAlignment=Enum.TextXAlignment.Left}) end
...
mk("UIPadding", content, {PaddingTop=UDim.new(0,18), PaddingLeft=UDim.new(0,18), PaddingRight=UDim.new(0,18)}); mk("UIListLayout", content, {Padding=UDim.new(0,16)})

local function makeButton(txt)
	local b=mk("TextButton", content, {Size=UDim2.new(0,260,0,44), BackgroundColor3=color.card, Text=txt, BorderSizePixel=0, Font=Enum.Font.GothamSemibold, TextSize=18, TextColor3=Color3.fromRGB(235,235,240)})
	mk("UICorner", b, {CornerRadius=UDim.new(0,8)}); local st=mk("UIStroke", b, {Transparency=1, Thickness=1.4, Color=Color3.fromRGB(140,140,150)}); hover(b,color.card,color.cardHover); return b,st
end
...
local track = mk("Frame", slider, {AnchorPoint=Vector2.new(0.5,0.5), Position=UDim2.new(0.5,0,0.5,0), Size=UDim2.new(1,0,0,6), BackgroundColor3=color.track, BorderSizePixel=0})
...
mk("UICorner", knob, {CornerRadius=UDim.new(1,0)}); mk("UIStroke", knob, {Color=Color3.fromRGB(120,120,130), Transparency=0.2, Thickness=1.2}); hover(knob,color.card,color.cardHover)
...
local tick = mk("Frame", chk, {Visible=true, Size=UDim2.new(0,10,0,10), Position=UDim2.new(0.5,-5,0.5,-5), BackgroundColor3=Color3.fromRGB(230,230,240), BorderSizePixel=0})
mk("UICorner", tick, {CornerRadius=UDim.new(0,2)}); hover(chk,color.card,color.cardHover)
mk("TextLabel", chkRow, {Position=UDim2.new(0,26,0,0), Size=UDim2.new(1,-26,1,0), BackgroundTransparency=1, Text="check", Font=Enum.Font.Gotham, TextSize=14, TextColor3=color.muted, TextXAlignment=Enum.TextXAlignment.Left})

-- drag window
do local dragging,startPos,startInput
	header.InputBegan:Connect(function(input)
		if input.UserInputType==Enum.UserInputType.MouseButton1 then dragging=true; startPos=win.Position; startInput=input.Position
			input.Changed:Connect(function() if input.UserInputState==Enum.UserInputState.End then dragging=false end end)
		end end)
	UserInputService.InputChanged:Connect(function(input)
		if dragging and input.UserInputType==Enum.UserInputType.MouseMovement then local d=input.Position-startInput; win.Position=UDim2.new(startPos.X.Scale,startPos.X.Offset+d.X,startPos.Y.Scale,startPos.Y.Offset+d.Y) end end)
end

-- UI -> Logic hooks
local shadeEnabled=true
local function toggleShade(on) shadeEnabled=on end
btnHoverShade.MouseButton1Click:Connect(function() toggleShade(not shadeEnabled) end)
local outlineEnabled=false
local function setOutline(on) outlineEnabled=on; outlineStroke.Transparency = on and 0.15 or 1; hoverStroke.Transparency = on and 0.15 or 1; updateESPOutline(on) end
btnOutline.MouseButton1Click:Connect(function() setOutline(not outlineEnabled) end)

-- slider (map 0..1 -> 60..140)
local dragging=false; local sliderValue=0.5
local function setSlider(alpha) alpha=math.clamp(alpha,0,1); sliderValue=alpha; knob.Position=UDim2.new(alpha,-9,0.5,-9); TP_STEP_STUDS = math.floor(60 + alpha*(140-60)) end
setSlider( (TP_STEP_STUDS-60)/(140-60) )
knob.InputBegan:Connect(function(i) if i.UserInputType==Enum.UserInputType.MouseButton1 then dragging=true end end)
UserInputService.InputEnded:Connect(function(i) if i.UserInputType==Enum.UserInputType.MouseButton1 then dragging=false end end)
UserInputService.InputChanged:Connect(function(i)
	if dragging and i.UserInputType==Enum.UserInputType.MouseMovement then local x=(i.Position.X - track.AbsolutePosition.X)/track.AbsoluteSize.X; setSlider(x) end end)

-- checkbox -> AUTO_ENABLED
	...
	while true do
		...
		if not node then
			...
			if not collectViaRemote(node.info, node.part, 1.2) then
				...
				if not done then cooldown(node.part, 3.0) end
			end
			tpTo(HOME_POS)
		end
	end
...
LP.CharacterAdded:Connect(function(c)
	Char=c; HRP=c:WaitForChild("HumanoidRootPart"); task.delay(0.2, function() useSwordDebounced() end)
end)
